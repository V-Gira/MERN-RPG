...
const gameByID = async (req, res, next, id) => {
  try {
    let game = await Game.findById(id).populate('gameMaster', '_id name')
    if (!game)
      return res.status('400').json({
...
const isGameMaster = (req, res, next) => {
    const isGameMaster = req.game && req.auth && req.game.gameMaster._id == req.auth._id
    if(!isGameMaster){
...
const listByGameMaster = (req, res) => {
  Game.find({gameMaster: req.profile._id}, (err, games) => {
    if (err) {
      return res.status(400).json({
        error: errorHandler.getErrorMessage(err)
      })
